...
bool32 IsBattlerProtected(u32 battler, u32 move)
{
    ...
    // Z-Moves and Max Moves and Piercing Drill bypass protection (except Max Guard).
    if ((IsMaxMove(move) || gBattleStruct->zmove.active || ((GetBattlerAbility(gBattlerAttacker) == ABILITY_PIERCING_DRILL) && gBattleMoves[move].makesContact))
...
static inline u32 CalcMoveBasePower(u32 move, u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 weather)
{
    ...
    case EFFECT_WEATHER_BALL:
        if ((weather & B_WEATHER_ANY) || (gBattleMons[battlerAtk].ability == ABILITY_MEGA_SOL))
            basePower *= 2;
        break;
    case EFFECT_PURSUIT:
...
static inline u32 CalcMoveBasePowerAfterModifiers(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case EFFECT_SOLAR_BEAM:
        if ((IsBattlerWeatherAffected(battlerAtk, (B_WEATHER_HAIL | B_WEATHER_SANDSTORM | B_WEATHER_RAIN | B_WEATHER_SNOW))) && !(gBattleMons[battlerAtk].ability == ABILITY_MEGA_SOL))
...
static uq4_12_t GetWeatherDamageModifier(u32 battlerAtk, u32 move, u32 moveType, u32 holdEffectAtk, u32 holdEffectDef, u32 weather)
{
    ...
    if ((weather & B_WEATHER_SUN) || gBattleMons[battlerAtk].ability == ABILITY_MEGA_SOL)
    {
        if (moveType != TYPE_FIRE && moveType != TYPE_WATER)
            return UQ_4_12(1.0);
...
static inline uq4_12_t GetPiercingMoveAgainstProtectionModifier(u32 battlerAtk, u32 battlerDef, u32 move)
{
    if ((gBattleStruct->zmove.active || IsMaxMove(move) || ((gBattleMons[battlerAtk].ability == ABILITY_PIERCING_DRILL) && gBattleMoves[move].makesContact)) && IS_BATTLER_PROTECTED(battlerDef))
        return UQ_4_12(0.25);
    return UQ_4_12(1.0);
}
                            ...
                            u32 holdEffectAtk, u32 holdEffectDef, u32 abilityAtk, u32 abilityDef)
{
    ...
    DAMAGE_APPLY_MODIFIER(GetPiercingMoveAgainstProtectionModifier(battlerAtk, battlerDef, move));
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    if (gBattleWeather & B_WEATHER_STRONG_WINDS && WEATHER_HAS_EFFECT && !(abilityAtk == ABILITY_MEGA_SOL))
    {
